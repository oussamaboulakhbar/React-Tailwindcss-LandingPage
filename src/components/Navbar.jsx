    ...
    return (
        <header className='w-full bg-white md:bg-transparent fixed top-0 left-0 right-0'>
            <nav className={`py-4 lg:px-14 px-4 ${isSticky ? "sticky top-0 left-0 right-0 border-b bg-white duration-300" : ""}`}>
                    ...
                    <ul className='md:flex space-x-12 hidden'>
                        {
                            navItems.map(({ link, path }) => <Link to={path} spy={true} smooth={true}
                                key={path} className='block text-base text-gray900 hover:text-brandPrimary cursor-pointer
                ...
                <div className={`space-y-4 px-4 mt-16 py-7 text-center bg-brandPrimary ${isMenuOpen ? "block fixed top-0 right-0 left-0 " : "hidden"}`}>
                    {
                        navItems.map(({ link, path }) => <Link to={path} spy={true} smooth={true}
                            offset={-100} key={path} className='block text-base text-gray900 hover:text-neutral-600 cursor-pointer
                                first:font-medium' >{link}</Link>)
                    }
